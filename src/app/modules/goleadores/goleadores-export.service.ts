...
export class GoleadoresExportService {
  ...
  async descargarImagen(data: GoleadoresExportables): Promise<void> {
    const url = URL.createObjectURL(new Blob([this.svg(data)], { type: 'image/svg+xml;charset=utf-8' }));
    try {
