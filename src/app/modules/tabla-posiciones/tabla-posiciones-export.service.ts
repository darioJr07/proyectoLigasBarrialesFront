...
export class TablaPosicionesExportService {
  ...
  async descargarImagen(data: TablaExportable, escudos: Record<number, string | undefined> = {}): Promise<void> {
    ...
    const url = URL.createObjectURL(new Blob([this.svg(data, escudosEmbebidos)], { type: 'image/svg+xml;charset=utf-8' }));
    try {
  ...
  private svg(data: TablaExportable, escudos: Record<number, string>): string {
